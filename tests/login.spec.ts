  ...
  runTest() {
    test.describe("Home Page Functionality Test", () => {
      ...
      test("Verify that a user can log in with valid credentials TS_LOG_02", async ({
        ...
        await runner.clickOnElement(homePage.navbarLogin);
        await runner.verifyContainText(loginModal.loginModalLabel,'Log in')
        await runner.fillInputBox(loginModal.usernameTextField,envData.username)
        await runner.fillInputBox(loginModal.passwordTextField,envData.password)
      ...
      test("Verify that a user cannot log in with invalid credentials TS_LOG_03", async ({
        ...
        await runner.clickOnElement(homePage.navbarLogin);
        await runner.verifyContainText(loginModal.loginModalLabel,'Log in')
        await runner.fillInputBox(loginModal.usernameTextField,fakeUser.username)
        await runner.fillInputBox(loginModal.passwordTextField,fakeUser.password)
